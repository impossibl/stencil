...
public class StencilOperands {
   ...
   * 
   * @param bigdContext
   *          the math context instance to use for +,-,/,*,% operations on big
   *          decimals.
   * @param bigdScale
  ...
  /**
   * Given a BigInteger, narrow it to an Integer or Long if it fits and the
   * arguments class allow it.
   * <p>
   * The rules are: if either arguments is a BigInteger, no narrowing will occur
   * if either arguments is a Long, no narrowing to Integer will occur
   * </p>
   * 
   * @param lhs
   *          the left hand side operand that lead to the bigi result
   * @param rhs
   *          the right hand side operand that lead to the bigi result
   * @param bigi
  ...
  /**
   * Given a BigDecimal, attempt to narrow it to an Integer or Long if it fits
   ...
   * <ul>
   * <li>If all objects are of the same type, the array returned will be an
   * array of that same type</li>
   * <li>If all objects are Numbers, the array returned will be an array of
   * Numbers</li>
   * <li>If all objects are convertible to a primitive type, the array returned
   * will be an array of the primitive type</li>
   * </ul>
   * 
   * @param untyped
   *          an untyped array
   * @return the original array if the attempt to strictly type the array fails,
   *         a typed array otherwise
   */
   ...
   * <p>
   * If any numeric add fails on coercion to the appropriate type, treat as
   * Strings and do concatenation.
   * </p>
   * 
   * @param left
   *          first value
  ...
  public Object add(Object left, Object right, ParserRuleContext errCtx) {
    ...
    if (left instanceof Collection<?> || right instanceof Collection<?>) {
      List<Object> res = newArrayList();
      res.addAll(toCollection(left));
      res.addAll(toCollection(right));
      return res;
    }
    ...
    // if both are maps... concatenate them
    if (left instanceof Map<?, ?> || right instanceof Map<?, ?>) {
      Map<Object, Object> res = newHashMap();
      res.putAll((Map<?, ?>) left);
      res.putAll((Map<?, ?>) right);
      return res;
    }
    ...
    try {
      ...
      // if either are numbers use integers
  ...
  protected int compare(Object left, Object right, String operator) {
    if (left != null && right != null) {
      if (left instanceof BigDecimal || right instanceof BigDecimal) {
        ...
        // OK because of instanceof check above
   ...
   */
  public Collection<?> toCollection(Object val) {
   ...
   * method calls, e.g. a call to substring(int,int) with an int and a long will
   * fail, but a call to substring(int,int) with an int and a short will
   * succeed.
   * 
   * @param original
   *          the original number.
  ...
  /**
   * Given a Number, return back the value attempting to narrow it to a target
   * class.
   * 
   * @param original
   *          the original number
  ...
  public Object leftShift(Object val, Object shift) {
    return toInteger(val) << toInteger(shift);
  }
  
  public Object rightShift(Object val, Object shift) {
    return toInteger(val) >> toInteger(shift);
  }
